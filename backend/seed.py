...
async def seed_demo_data(db):
    ...
    try:
        ...
        # Franchise tiers (V2.1) — already seeded above by the idempotent top-up
        # block. Just build the name → id map for franchise tier assignment.
        tier_by_name = {}
        for doc in await db.franchise_tiers.find({}, {"_id": 0}).to_list(50):
            tier_by_name.setdefault(doc["name"], doc["id"])
