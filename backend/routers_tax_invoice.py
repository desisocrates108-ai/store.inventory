...
async def tax_invoice_pdf(tid: str, show_discount: bool = False,
                          user: dict = Depends(get_current_user)):
    """Render the Tax Invoice PDF.

    `show_discount` (default False) controls customer-facing presentation:
    - False → Discount % column + Total Discount row are HIDDEN; the
              line-item Rate column shows the **post-discount effective rate**
              so qty × rate reconciles to the printed amount.
              Use this for the customer copy.
    - True  → Internal view: shows original Rate, Disc%, and Total Discount.

    The underlying invoice data is NEVER recalculated — only PDF rendering changes.
    """
    inv = await _db.tax_invoices.find_one({"id": tid}, {"_id": 0})
    if not inv:
        raise HTTPException(404, "Not found")
    org = await get_org_settings_doc()
    pdf = _render_tax_invoice_pdf(inv, org, show_discount=show_discount)
    name = (inv.get("invoice_number") or "draft").replace("/", "-")
    suffix = "" if show_discount else "-customer"
...
def _render_tax_invoice_pdf(inv: dict, org: dict, show_discount: bool = False) -> bytes:
    """Professional GST-compliant Tax Invoice PDF via reportlab.

    When `show_discount=False` (customer copy), the Disc% column and the
    Total Discount totals row are omitted. The Rate column instead shows the
    effective per-unit rate (unit_price × (1 - discount/100)), so the printed
    line totals reconcile cleanly for the customer without exposing the
    discount %. The underlying invoice data (`discount_percent`, `taxable_value`,
    grand_total, etc.) is unchanged."""
    ...
    if show_discount:
        ...
        header_row = ["#", "Description", "HSN", "Qty", "Unit", "Rate", "Taxable"]
    if is_inter:
    ...
    for i, li in enumerate(inv.get("line_items", []) or [], 1):
        ...
        gross_rate = float(li.get("unit_price", 0) or 0)
        disc_pct = float(li.get("discount_percent", 0) or 0)
        # In customer mode, hide the discount % by showing the post-discount effective rate.
        rate_display = gross_rate if show_discount else round(gross_rate * (1 - disc_pct / 100), 2)
        row = [
            str(i),
        ...
        if show_discount:
            row.append(f"{disc_pct:.0f}%")
        row.append(f"{li.get('taxable_value', 0):.2f}")
        if is_inter:
            row += [
    ...
    if show_discount:
        ...
        # Without Disc% column we have one fewer column → expand description.
        if is_inter:
            col_widths = [6 * mm, 72 * mm, 14 * mm, 13 * mm, 11 * mm, 18 * mm, 20 * mm, 18 * mm, 22 * mm]
        else:
            col_widths = [6 * mm, 60 * mm, 14 * mm, 12 * mm, 11 * mm, 16 * mm, 19 * mm, 18 * mm, 18 * mm, 22 * mm]
    ...
    if show_discount and (inv.get("total_discount") or 0) > 0:
