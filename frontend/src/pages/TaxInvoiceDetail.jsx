...
export default function TaxInvoiceDetail() {
  ...
  const [showDiscount, setShowDiscount] = useState(() => {
    try {
      const v = sessionStorage.getItem("tax_invoice_show_discount");
      return v === "true";
    } catch (_) { return false; }
  });
  React.useEffect(() => {
    try { sessionStorage.setItem("tax_invoice_show_discount", String(showDiscount)); } catch (_) { /* noop */ }
  }, [showDiscount]);
  ...
  const downloadPdfFile = async () => {
    ...
    const suffix = showDiscount ? "" : "-customer";
    await downloadPdf(
      `/tax-invoices/${inv.id}/pdf?show_discount=${showDiscount ? "true" : "false"}`,
      `tax-invoice-${safeNum}${suffix}.pdf`,
    );
  };
  ...
  const shareWhatsApp = () => {
    if (!inv?.id) { toast.error("Save the draft first"); return; }
    const phone = (inv.contact_phone || "").replace(/\D/g, "");
    const msg = [
      `Hi ${inv.billing_name || "there"},`,
      `Tax Invoice ${inv.invoice_number || "(draft)"} dated ${inv.invoice_date || ""}.`,
      `Grand Total: ₹${Number(inv.grand_total || 0).toFixed(2)}.`,
      `Payment Terms: ${inv.payment_terms || "Net 30"}.`,
      `${showDiscount ? "" : "Customer copy — discount not shown."}`.trim(),
    ].filter(Boolean).join("\n");
    const url = phone
    ...
    <div className="space-y-6" data-testid="tax-invoice-detail-page">
      <div className="flex items-start justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {inv.id && (
            <>
              <Button variant="outline" onClick={downloadPdfFile} data-testid="download-pdf-btn">
                <FilePdf size={14} className="mr-2" /> PDF
              </Button>
              <Button variant="outline" onClick={printPdf} data-testid="print-pdf-btn">
                <Printer size={14} className="mr-2" /> Print
              </Button>
              <Button variant="outline" onClick={shareWhatsApp} data-testid="whatsapp-btn">
                <WhatsappLogo size={14} className="mr-2" /> WhatsApp
              </Button>
              <Button variant="outline" onClick={emailViaMailto} data-testid="email-btn">
                <EnvelopeSimple size={14} className="mr-2" /> Email
              </Button>
              <label
                className="flex items-center gap-2 rounded-md border border-border bg-card px-3 py-1.5 text-xs cursor-pointer hover:bg-muted/40 select-none"
                title={showDiscount ? "Customer PDF will INCLUDE discount" : "Customer PDF will HIDE discount (recommended for customer copy)"}
                data-testid="show-discount-toggle-wrap"
              >
                <EyeSlash size={12} className={showDiscount ? "text-muted-foreground" : "text-foreground"} />
                <span className="font-medium">{showDiscount ? "Show Discount" : "Hide Discount on Customer Copy"}</span>
                <Switch
                  checked={showDiscount}
                  onCheckedChange={setShowDiscount}
                  data-testid="show-discount-toggle"
                />
              </label>
            </>
          )}
